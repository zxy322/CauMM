...
def train(args, model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config,
    ...
    metric_logger.add_meter('loss_V', utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    ...
    for i, batch in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        labels = batch['label']
        texts = batch['caption']
        fake_image_boxes = batch['fake_image_box'].to(device, non_blocking=True)
        fake_word_pos = batch['fake_text_pos'].to(device, non_blocking=True)
